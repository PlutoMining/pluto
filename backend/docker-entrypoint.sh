  ...
  if [ -d "$module_path/node_modules" ]; then
    ...
    case "$module_name" in
      ...
      db)
        ...
        ;;
      utils)
        # Check if it has runtime dependencies
        if [ ! -f "$module_path/package.json" ] || ! grep -q '"dependencies"' "$module_path/package.json" 2>/dev/null; then
          echo "Dependencies already installed in common/$module_name"
          return 0
        fi
        ;;
      interfaces)
        # This module might not have runtime dependencies
